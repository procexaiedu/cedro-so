...
export async function getPatients(
  ...
  try {
    ...
    const totalCount = typeof count === 'number' ? count : patients.length
    ...
    const totalPages = pagination.limit > 0 ? Math.ceil(totalCount / pagination.limit) : 0
...
export async function getPatientStats(therapistId?: string): Promise<PatientStats> {
  try {
    // Get all patients from cedro.patients table
    let patientQuery = supabase
      .schema('cedro')
      .from('patients')
      .select('id, is_on_hold', { count: 'exact' })

    if (therapistId) {
      patientQuery = patientQuery.eq('therapist_id', therapistId)
    }

    const { count: patientTotal, data: patientsList, error: patientError } = await patientQuery

    if (patientError || !patientsList) {
    ...
    // Get all appointments to calculate derived status and counts
    ...
    allAppointments?.forEach((apt: any) => {
      ...
      appointmentCounts.set(
        apt.patient_id,
        (appointmentCounts.get(apt.patient_id) || 0) + 1
      )
    })
    ...
    patientsList.forEach((p: any) => {
      const lastApptList = appointmentsByPatient.get(p.id)
      const lastAppointment = lastApptList?.[0]?.start_at ?? null
    ...
    const totalAppointments = Array.from(appointmentCounts.values()).reduce((sum, count) => sum + count, 0)

    // Get active therapists count (only if not filtered by therapist)
    let therapistCount = 1
    if (!therapistId) {
      const { count: therapistCountFromDb, error: therapistError } = await supabase
        .schema('cedro')
        .from('users')
        .select('id', { count: 'exact', head: true })
        .in('role', ['therapist', 'admin'])

      if (!therapistError && therapistCountFromDb) {
        therapistCount = therapistCountFromDb
      }
    }

    return {
      totalPatients: patientTotal || 0,
 ...
 */
export function getPatientStatus(patient: Pick<Patient, 'is_on_hold' | 'last_appointment'>): PatientStatus {
