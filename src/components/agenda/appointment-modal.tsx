...
export function AppointmentModal({
  ...
  const handleServiceChange = (serviceId: string) => {
    ...
    validateField('service_id', serviceId)
  }
  ...
  const validateField = (field: string, value: any) => {
    ...
    switch (field) {
      case 'patient_id':
        if (!value) {
          errors.patient_id = 'Paciente é obrigatório'
        } else {
      ...
      case 'therapist_id':
        if (!value) {
          errors.therapist_id = 'Terapeuta é obrigatório'
        } else {
          delete errors.therapist_id
        }
        break
      case 'start_time':
        if (!value) {
          errors.start_time = 'Horário é obrigatório'
        } else {
      ...
      case 'duration':
        if (!value || value < 15) {
          ...
          errors.duration = 'Duração máxima é 480 minutos'
        } else {
  ...
  const handleDelete = async () => {
    ...
    // Se não foi confirmado, mostra o diálogo de confirmação
    if (!showDeleteConfirm) {
      setShowDeleteConfirm(true)
      return
    }

    // Se foi confirmado, executa a deleção
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <>
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    validateField('patient_id', value)
                  }}
                >
                  <SelectTrigger className={validationErrors.patient_id ? 'border-red-500' : ''}>
                ...
                {validationErrors.patient_id && (
                  <p className="text-sm text-red-500">{validationErrors.patient_id}</p>
                )}
              </>
            )}
          </div>
              ...
              <>
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    validateField('therapist_id', value)
                  }}
                >
                  <SelectTrigger className={validationErrors.therapist_id ? 'border-red-500' : ''}>
                ...
                {validationErrors.therapist_id && (
                  <p className="text-sm text-red-500">{validationErrors.therapist_id}</p>
                )}
              </>
            )}
          </div>
                ...
                <>
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      validateField('start_time', e.target.value)
                    }}
        ...
        {showDeleteConfirm && appointment && (
          <div className="bg-red-50 border-l-4 border-red-500 p-4 my-4 rounded">
            <h3 className="font-bold text-red-900 mb-2">Confirmar exclusão</h3>
            <p className="text-sm text-red-800 mb-4">
              Você tem certeza que deseja excluir este agendamento?
            </p>
            <p className="text-sm text-red-700 bg-white p-2 rounded mb-4 border border-red-200">
              <span className="font-semibold">{appointment.patient_name}</span> - {format(parseISO(appointment.start_at), 'dd/MM/yyyy HH:mm', { locale: ptBR })}
            </p>
        ...
        <div className="flex justify-between pt-4">
          <div>
            {(isEditing || mode === 'view') && appointment && !showDeleteConfirm && (
              <Button
                variant="destructive"
                onClick={handleDelete}
                disabled={deleteMutation.isPending}
          ...
          <div className="flex gap-2">
            <Button
              ...
              disabled={createMutation.isPending || updateMutation.isPending || deleteMutation.isPending || showDeleteConfirm}
            >
            ...
            {!isReadOnly && !showDeleteConfirm && (
              <Button
                onClick={handleSave}
                disabled={createMutation.isPending || updateMutation.isPending || loadingLinkedPatients || loadingLinkedTherapists || Object.keys(validationErrors).length > 0}
              >
