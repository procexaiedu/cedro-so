...
#### 🔧 **KPI Metrics Validation via Supabase MCP** ✅
- **Status:** Fixed critical data mismatch
- **Problem:** Stats cards on `/pacientes` showed 0 patients instead of 78
- **Root Cause:** Function `getPatientStats()` queried non-existent columns in view `vw_patient_overview`
  - View has: `patient_id, full_name, email, phone, current_therapist_id, current_therapist_name, active_plans, upcoming_appointments`
  - Code expected: `patient_id, is_on_hold, last_appointment, total_appointments` ❌
- **Solution Implemented:**
  - Rewrote `getPatientStats()` in `src/data/pacientes.ts` to query `cedro.patients` table directly
  - Added aggregation of appointments to calculate derived status (active/inactive/suspended)
  - Maintains schema `cedro` usage throughout
- **Validation Results (via Supabase MCP):**
  - Total Patients: **78** ✅
  - Status Distribution: suspended=0, active=0, inactive=78 ✅
  - Appointments Today: **22** ✅
  - Revenue This Month: **R$ 0,00** ✅
  - Overdue Invoices: **0** ✅
- **Files Modified:**
  - `src/data/pacientes.ts` - getPatientStats() function
  - Dashboard metrics now accurately reflect database state
- See: [Recent Updates](./05-recent-updates.md#-kpi-metrics-validation-via-supabase-mcp)
